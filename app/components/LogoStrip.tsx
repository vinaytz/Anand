        ...
        {[...logos, ...logos].map((logo, idx) => (
          <React.Fragment key={idx}>
            <div className="flex items-center shrink-0 px-8 md:px-12">
              <span className="text-xl md:text-2xl font-serif text-white/20 hover:text-[#d4af37]/60 transition-colors duration-700 whitespace-nowrap cursor-default select-none">
                {logo}
              </span>
            </div>
...
export default function LogoStrip() {
  ...
  return (
    <motion.section
      ref={ref}
      style={{ opacity }}
      className="relative py-10 md:py-14 border-y border-white/[0.04] bg-[#030303] overflow-hidden"
    >
      {/* Label */}
