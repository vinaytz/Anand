...
export default function KeynoteShowcase() {
  return (
    <section className="py-32 px-6 md:px-12 bg-[var(--th-bg)] relative overflow-hidden transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        <div className="mb-20 space-y-4">
            <span className="text-gold-500 uppercase tracking-[0.2em] text-sm font-medium">Signature Talks</span>
            <h2 className="text-4xl md:text-6xl font-serif text-[var(--th-text)] max-w-3xl">
        ...
        <div className="space-y-24">
            {keynotes.map((talk, index) => (
                <div key={index} className="group relative grid grid-cols-1 lg:grid-cols-12 gap-12 items-center border-t border-[var(--th-border)] pt-16">
                    {/* Background Glow */}
                    ...
                    <div className="lg:col-span-7 space-y-8">
                        <div>
                            <span className="text-sm font-mono text-[var(--th-text-4)] mb-2 block">0{index + 1}</span>
                            <h3 className="text-4xl md:text-5xl font-serif text-[var(--th-text)] mb-2">{talk.title}</h3>
                            <p className="text-xl text-gold-500/80 font-medium italic">{talk.subtitle}</p>
                        </div>
                        <p className="text-[var(--th-text-3)] leading-relaxed text-lg max-w-2xl">
                            {talk.description}
                        </p>
                        
                        <div className="flex flex-wrap gap-4 pt-4">
                             <div className="px-4 py-2 bg-[var(--th-border)] border border-[var(--th-border-hover)] rounded-full text-xs uppercase tracking-widest text-[var(--th-text-2)]">
                                Best for: {talk.audience}
                             </div>
                        </div>

                        <button className="flex items-center gap-3 text-[var(--th-text)] border-b border-gold-500 pb-1 hover:text-gold-400 transition-colors group/btn">
                            Book this Talk 
                    ...
                    <div className="lg:col-span-5 relative">
                        <div className="bg-[var(--th-elevated)] border border-[var(--th-border)] p-8 rounded-sm relative z-10 group-hover:border-gold-500/30 transition-colors duration-500">
                           <h4 className="text-[var(--th-text)] font-serif text-xl mb-6">Key Takeaways</h4>
                           <ul className="space-y-4">
                               {talk.takeaways.map((item, i) => (
                                   <li key={i} className="flex items-start gap-4">
                                       <CheckCircle2 className="w-5 h-5 text-gold-500 shrink-0 mt-0.5" />
                                       <span className="text-[var(--th-text-3)] font-light">{item}</span>
                                   </li>
                               ))}
                           </ul>
                        </div>
                        {/* Decorative Box Shadow */}
                        <div className="absolute top-4 left-4 w-full h-full border border-[var(--th-border-subtle)] -z-0" />
                    </div>
                </div>
            ))}
        </div>
